...
REQUIRED_ENV_KEYS=(
  MINDCART_MYSQL_ROOT_PASSWORD MINDCART_MYSQL_PASSWORD
  MINDCART_REDIS_PASSWORD MINDCART_RABBIT_PASSWORD
  MINDCART_INTERNAL_TOKEN MINDCART_GATEWAY_TOKEN MINDCART_JWT_SECRET
  MINDCART_MODEL_CRYPTO_KEY MINDCART_GRAFANA_PASSWORD
  MINDCART_POSTGRES_PASSWORD
)
...
append_missing_env_keys() {
  ...
  for key in "${REQUIRED_ENV_KEYS[@]}"; do
    if ! grep -q "^${key}=" "$ENV_FILE"; then
      case "$key" in
        MINDCART_MODEL_CRYPTO_KEY) val=$(openssl rand -base64 32 | tr -d '\n') ;;
        *) val=$(rand_hex 24) ;;
      esac
      printf '%s=%s\n' "$key" "$val" >> "$ENV_FILE"
...
# MindCart 运行时密钥（本文件被 git 忽略，禁止提交）
MINDCART_MYSQL_ROOT_PASSWORD=$(rand_hex 12)
# 业务账号唯一密码变量：compose 传给 mysql-init 建号、后端连接、recon 对账共用
# （旧版曾拆成 APP/MYSQL 两个名字互相脱节，实际账号被建成空密码——两个 bug 恰好抵消）
MINDCART_MYSQL_PASSWORD=$(rand_hex 12)
MINDCART_MYSQL_USER=mindcart
MINDCART_MYSQL_HOST=127.0.0.1
MINDCART_MYSQL_PORT=33306
MINDCART_REDIS_HOST=127.0.0.1
MINDCART_REDIS_PORT=26379
MINDCART_REDIS_PASSWORD=$(rand_hex 12)
MINDCART_RABBIT_HOST=127.0.0.1
MINDCART_RABBIT_PORT=5774
MINDCART_RABBIT_USER=mindcart
MINDCART_RABBIT_PASSWORD=$(rand_hex 12)
MINDCART_RABBIT_VHOST=/
MINDCART_NACOS_ADDR=127.0.0.1:28848
MINDCART_NACOS_USERNAME=nacos
MINDCART_NACOS_PASSWORD=nacos
MINDCART_NACOS_GROUP=MINDCART_GROUP
# 双凭证（见 common/UserContextFilter）：集群内 Feign 调用凭证 + 网关转发凭证，
# /internal/** 只认前者，用户流量无论因何种路由缺陷到达内部接口都会被拒
MINDCART_INTERNAL_TOKEN=$(rand_hex 24)
MINDCART_GATEWAY_TOKEN=$(rand_hex 24)
MINDCART_JWT_SECRET=$(rand_hex 32)
MINDCART_MODEL_CRYPTO_KEY=$(openssl rand -base64 32 | tr -d '\n')
MINDCART_GRAFANA_PASSWORD=$(rand_hex 12)
MINDCART_FILE_DIR=$ROOT/run/files
MINDCART_FILE_BASE_URL=http://localhost:9080
# 模型密钥留空则使用库内配置；从 Smartlect run/model.env 复制可复用已有 Key
MINDCART_CHAT_API_KEY=
MINDCART_CHAT_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1
MINDCART_CHAT_MODEL=qwen-plus
MINDCART_EMBED_API_KEY=
MINDCART_EMBED_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1
MINDCART_EMBED_MODEL=text-embedding-v4
# mindcart-voice 的 PostgreSQL/pgvector（compose 里的 mall-postgres）
MINDCART_POSTGRES_HOST=127.0.0.1
MINDCART_POSTGRES_PORT=35432
MINDCART_POSTGRES_DB=mindcart_voice
MINDCART_POSTGRES_USER=mindcart
MINDCART_POSTGRES_PASSWORD=$(rand_hex 12)
EOF
  )
  chmod 600 "$ENV_FILE"
  say "已生成 $ENV_FILE（600 权限）。如需复用模型 Key：把 Smartlect run/model.env 的值填入 MINDCART_CHAT_API_KEY / MINDCART_EMBED_API_KEY"
}
...
# 存量数据卷自愈：旧版 init 脚本变量未传入容器，mindcart 账号被建成空密码。
# ALTER USER 幂等，每次 infra-up 都跑一遍，保证账号密码与 runtime.env 一致。
heal_mysql_app_user() {
  docker exec mall-mysql mysql -uroot -p"${MINDCART_MYSQL_ROOT_PASSWORD}" \
    -e "ALTER USER 'mindcart'@'%' IDENTIFIED BY '${MINDCART_MYSQL_PASSWORD}'; FLUSH PRIVILEGES;" \
    >/dev/null 2>&1 || die "MySQL 业务账号密码自愈失败（检查 MINDCART_MYSQL_* 变量与容器状态）"
...
start_svc() {
  ...
  local jar_path
  case "$name" in
    gateway) jar_path=$(svc_jar mindcart-gateway);;
    user)    jar_path=$(svc_jar mindcart-user) ;;
    goods)   jar_path=$(svc_jar mindcart-goods) ;;
    trade)   jar_path=$(svc_jar mindcart-trade) ;;
    ai)      jar_path=$(svc_jar mindcart-ai) ;;
    voice)   jar_path=$(svc_jar mindcart-voice) ;;
    *) die "未知服务 $name";;
  esac
...
check_smoke() {
  ...
  # mindcart-voice：健康 → 经网关 WS 握手（query token）→ 目录同步状态
  say "冒烟：voice 语音服务"
