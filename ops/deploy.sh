...
set -euo pipefail
REPO_DIR=${1:?repo dir}
ENV_FILE=${MINDCART_ENV_FILE:-/opt/mindcart/run/runtime.env}
HEALTH_URL=${MINDCART_HEALTH_URL:-http://127.0.0.1:9080/actuator/health}

echo "[deploy] 构建后端与前端"
(cd "$REPO_DIR/backend" && mvn -q -T 1C package -DskipTests)
(cd "$REPO_DIR/web" && npm ci --silent && npm run build)

echo "[deploy] 注入 runtime.env 并重启 mindcart-apps"
install -d /opt/mindcart/run
[ -f "$ENV_FILE" ] || { echo "缺少 $ENV_FILE"; exit 1; }
systemctl restart mindcart-apps
