...
import com.smartore.voice.service.ProfileReranker;
...
class EvalAblationTest {
    ...
    private A1 ablationRerank() throws Exception {
        ...
        for (Map<String, Object> g : golden) {
            ...
            var filter = com.smartore.voice.service.SqlFilterBuilder.fromSlots(slotsNoBudget);
    ...
    private A5 ablationSemanticRerank() throws Exception {
        ...
        var client = new com.smartore.voice.service.SemanticRerankClient(
                new com.smartore.voice.service.LlmGuard(
        ...
        var textBuilder = new com.smartore.voice.service.ProductTextBuilder();
        List<Map<String, Object>> golden = EvalSupport.loadGolden("eval/rag-golden.jsonl");
        ...
        for (Map<String, Object> g : golden) {
            ...
            Map<Long, com.smartore.voice.entity.ProductEntity> ents = fetchEntities(ids);
    ...
    private Map<Long, com.smartore.voice.entity.ProductEntity> fetchEntities(List<Long> ids)
            throws Exception {
        Map<Long, com.smartore.voice.entity.ProductEntity> out = new HashMap<>();
        try (var ps = pg.prepareStatement(
            ...
            try (var rs = ps.executeQuery()) {
                while (rs.next()) {
                    var e = new com.smartore.voice.entity.ProductEntity();
    ...
    private A3 ablationTtsDigits() throws Exception {
        ...
        for (boolean normalize : new boolean[]{false, true}) {
            ...
            for (String text : digitTexts) {
                ...
                // 不应计为回环错误
                String spokenRef = com.smartore.voice.voice.TtsTextNormalizer.normalize(text);
    ...
    private static com.smartore.voice.voice.AsrService ttsAsr(boolean unused) throws Exception {
        // 与 EvalVoiceLoopTest 同源修复：@Value 字段必须显式注入生产参数，
        // 否则 vad/flushGrace/settle 退化为 0，长句尾部被吞、CER 虚高
        return EvalSupport.reflectSet(new com.smartore.voice.voice.AsrService(), Map.of(
    ...
    private String transcribe(com.smartore.voice.voice.AsrService asr, byte[] pcm16k) throws Exception {
