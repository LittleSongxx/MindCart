...
import com.smartore.voice.compliance.ComplianceChecker;
import com.smartore.voice.dto.*;
import com.smartore.voice.entity.ProductEntity;
import com.smartore.voice.entity.SessionStateEntity;
import com.smartore.voice.entity.StreamChunk;
import com.smartore.voice.event.VoiceEventPublisher;
import com.smartore.voice.memory.ShortTermMemory;
import com.smartore.voice.memory.TurnSummarizer;
import com.smartore.voice.repository.ProductRepository;
import com.smartore.voice.voice.SentenceAggregator;
...
public class OrchestratorService {
    ...
    private final com.smartore.voice.integration.KnowledgeAnswerService knowledgeAnswerService;
    private final com.smartore.voice.integration.OrderQueryService orderQueryService;
    ...
    private EmotionResult doHandle(String sessionId, Long userId, String utterance) {
        ...
        try {
            ...
            if (com.smartore.voice.integration.OrderQueryService.looksLikeOrderQuery(utterance)) {
    ...
    private EmotionResult handleOrderConfirm(String sessionId, Long userId,
        ...
        if (pending != null) {
            ...
            if (containsYes(utterance)) {
                // 交易事实只在 Smartore 业务服务产生：发动作帧给前端代执行，
    ...
    private volatile java.util.List<com.smartore.voice.integration.KnowledgeClient.KnowledgeHitVO>
            lastKnowledgeEvidence = java.util.List.of();

    public java.util.List<com.smartore.voice.integration.KnowledgeClient.KnowledgeHitVO> lastKnowledgeEvidence() {
