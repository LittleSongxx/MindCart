 ...
 * 以用户自己的登录态走 Smartore 现成链路（加购 → /shopOrder/create），
...
public class VoiceOrderService {
    ...
    private final com.smartore.goods.api.GoodsFeignClient goodsClient;
    private final com.smartore.voice.repository.VoiceOrderEventRepository orderEventRepo;
    ...
    public Map<String, Object> cancelLastOrder(String sessionId) {
        ...
        Long userId = orderEventRepo.findFirstByOrderNoAndStatus(orderNo, "SUCCEEDED")
                .map(com.smartore.voice.entity.VoiceOrderEventEntity::getUserId).orElse(null);
    ...
    private void recordEvent(String sessionId, Long userId, String action, String requestId, String orderNo,
        ...
        try {
            var e = new com.smartore.voice.entity.VoiceOrderEventEntity();
    ...
    public String completeOnResult(String sessionId, String requestId,
        ...
        if (success) {
            ...
            // Smartore 单号形如 OD20260922024650764367：前 6 位是日期前缀，口播要的是**末 6 位**随机串
