...
import com.smartore.voice.integration.ModelConfigClient;
...
public class LlmConfigResolver {
    ...
    private Map<String, ModelConfigClient.ActiveModelConfigVO> fetchWithRetry() {
        ...
        log.warn("smartore-ai 模型配置 {} 次拉取均失败，voice 本次运行用本地 yml 兜底", FETCH_MAX_ATTEMPTS);
        return Map.of();
    }
    ...
    private Map<String, ModelConfigClient.ActiveModelConfigVO> fetch() {
        try {
            ...
            if (res != null && "200".equals(res.getCode()) && res.getData() != null) {
                log.info("已从 smartore-ai 拉取模型配置：{}", res.getData().keySet());
                return res.getData();
            }
            log.warn("smartore-ai 模型配置返回异常（{}），voice 用本地 yml 兜底", res == null ? "null" : res.getMsg());
        } catch (Exception e) {
            log.warn("拉取 smartore-ai 模型配置失败（{}），voice 用本地 yml 兜底", e.getMessage());
        }
        return Map.of();
    }
