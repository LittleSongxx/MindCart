...
public class SqlFilterBuilder {
    ...
    public static Filter fromSlots(Map<String, Object> slots) {
        ...
        if (gender != null) {
            // IS NULL 兜底：部分品类（口红/耳机等）的商品 attributes 未标注 gender，
            // 严格 IN 过滤会把它们全部排除 → 性别指代场景（送女友/送妈妈）零结果。
            // 与 runningShoeFilter 的 terrain 处理同一原则：未标注视为可接受，只排除明确冲突的。
            frags.add("(attributes->>'gender' IS NULL OR attributes->>'gender' IN (?, 'unisex'))");
