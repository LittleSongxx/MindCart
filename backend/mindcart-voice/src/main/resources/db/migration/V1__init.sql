-- ============================================================================
-- mindcart-voice 数据库 Schema（PostgreSQL 16 + pgvector）
-- 来源：裁剪自 jc-voice-shopping V1，只保留语音服务自有的四类数据：
--   1. voice_product  —— MindCart 商品目录的同步副本 + 向量（事实以 goods 服务为准）
--   2. session / session_message / session_state —— 语音会话
--   3. user_profile_dynamic —— 行为累加的动态画像
-- 不建：商家/用户/订单/FAQ（用户与订单事实在 MindCart MySQL，经 /internal/** 读取）
-- ============================================================================
...
-- ============================================================================
-- 一、商品同步目录（MindCart goods 服务的只读副本 + embedding，由 CatalogSyncService 维护）
-- ============================================================================

CREATE TABLE voice_product (
                         id              BIGINT        PRIMARY KEY,   -- 与 MindCart product.id 一致（同步写入，不自增）
...
CREATE TABLE session (
                         ...
                         user_id             BIGINT        NOT NULL,                -- MindCart user.id
