 ...
 *   mvn test -pl mindcart-voice -P eval -Dtest=AgentScopeSmokeTest
 * key 复用 MindCart 底座的环境变量：MINDCART_CHAT_BASE_URL / MINDCART_CHAT_API_KEY / MINDCART_CHAT_MODEL。
 */
@Tag("eval")
class AgentScopeSmokeTest {
    ...
    void openAiCompatibleChatWorks() {
        String baseUrl = System.getenv("MINDCART_CHAT_BASE_URL");
        String apiKey = System.getenv("MINDCART_CHAT_API_KEY");
        String model = System.getenv("MINDCART_CHAT_MODEL");
        assumeTrue(baseUrl != null && apiKey != null && model != null,
                "未配置 MINDCART_CHAT_* 环境变量，跳过真实调用冒烟");
