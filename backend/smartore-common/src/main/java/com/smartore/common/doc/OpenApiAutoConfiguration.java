package com.smartore.common.doc;
...
public class OpenApiAutoConfiguration {
    ...
    public OpenAPI smartoreOpenApi(
            @Value("${spring.application.name:smartore}") String applicationName,
            @Value("${server.port:}") String port) {
        Info info = new Info()
                .title(applicationName + " 接口文档")
                .description("Smartore 智能商城 · " + applicationName + "。"
                        + "内部接口（/internal/**）不经网关暴露，因此不在本文档的对外语义内。"
                        + "需要登录的接口请在请求头带上 token（登录接口见 smartore-user）。");
        if (!port.isBlank()) {
