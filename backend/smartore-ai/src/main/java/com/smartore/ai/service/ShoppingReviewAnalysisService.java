...
import com.smartore.ai.entity.ReviewAnalysisRequest;
import com.smartore.ai.entity.ShoppingReviewAnalysis;
import com.smartore.common.exception.CustomException;
import com.smartore.ai.mapper.ShoppingReviewAnalysisMapper;
...
public class ShoppingReviewAnalysisService {
    ...
    public ShoppingReviewAnalysis generate(ReviewAnalysisRequest request) {
        ...
        com.smartore.goods.api.ProductVO product = unwrap(goodsClient.getProduct(request.getProductId()));
        if (ObjectUtil.isNull(product)) {
            throw new CustomException("5001", "商品不存在或已被删除");
        }

        List<com.smartore.goods.api.ProductReviewVO> reviews =
    ...
    private ShoppingReviewAnalysis buildAnalysis(com.smartore.goods.api.ProductVO product, List<com.smartore.goods.api.ProductReviewVO> reviews) {
        int positiveCount = 0;
        int neutralCount = 0;
        int negativeCount = 0;
        int totalRating = 0;
        ...
        for (com.smartore.goods.api.ProductReviewVO review : reviews) {
     ...
     */
    private void fillAiInsights(ShoppingReviewAnalysis analysis, List<com.smartore.goods.api.ProductReviewVO> reviews) {
