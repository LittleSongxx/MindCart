...
public class AiChatService {
    ...
    /** LLM 调用观测：mindcart.llm.call{kind, outcome}，Grafana 看板与告警的数据源 */
    private <T> T timed(String kind, String model, java.util.function.Supplier<T> action) {
        ...
        try {
            ...
            Timer.builder("mindcart.llm.call")
                    .tag("kind", kind)
    ...
    private ChatCompletionResult chatCompletionInner(JSONArray messages, JSONArray tools) {
        ...
        try {
            ...
            if (meterRegistry != null) {
                meterRegistry.counter("mindcart.llm.tokens", "type", "prompt").increment(promptTokens);
                meterRegistry.counter("mindcart.llm.tokens", "type", "completion").increment(completionTokens);
            }
