...
import com.mindcart.ai.mapper.ProductKnowledgeMapper;
import com.mindcart.ai.mapper.ProductKnowledgeChunkMapper;
import com.mindcart.ai.mapper.ProductKnowledgeEmbeddingMapper;
...
public class ProductKnowledgeService {
    ...
    private com.mindcart.goods.api.KnowledgeSourceVO knowledgeSource(Integer productId) {
        com.mindcart.common.result.Result<com.mindcart.goods.api.KnowledgeSourceVO> result =
    ...
    private com.mindcart.goods.api.ProductVO loadProduct(Integer productId) {
        com.mindcart.common.result.Result<com.mindcart.goods.api.ProductVO> result = goodsClient.getProduct(productId);
        return (result == null || !"200".equals(result.getCode())) ? null : result.getData();
    }

    public void add(ProductKnowledge productKnowledge) {
        validate(productKnowledge);
        com.mindcart.goods.api.ProductVO product = loadProduct(productKnowledge.getProductId());
    ...
    public int importFromProduct(Integer productId) {
        ...
        com.mindcart.goods.api.ProductVO product = loadProduct(productId);
        if (ObjectUtil.isNull(product)) {
        ...
        int count = 0;
        int sort = 1;
        com.mindcart.goods.api.KnowledgeSourceVO source = knowledgeSource(productId);
        com.mindcart.goods.api.KnowledgeSourceVO detail = source;
        ...
        List<com.mindcart.goods.api.KnowledgeSourceVO.ParamEntry> params =
        ...
        if (!params.isEmpty()) {
            ...
            for (com.mindcart.goods.api.KnowledgeSourceVO.ParamEntry param : params) {
    ...
    public int importAll() {
        List<com.mindcart.goods.api.ProductVO> products = unwrap(goodsClient.searchOnSale(null, null, 50));
        int count = 0;
        for (com.mindcart.goods.api.ProductVO product : products) {
            try {
     ...
     */
    private void saveImported(com.mindcart.goods.api.ProductVO product, String knowledgeType, String title, String content, int sort) {
    ...
    public List<ProductKnowledge> selectAll(ProductKnowledge productKnowledge) {
        List<com.mindcart.ai.entity.ProductKnowledge> __rows = productKnowledgeMapper.selectAll(productKnowledge);
        nameFillService.fillProducts(__rows, com.mindcart.ai.entity.ProductKnowledge::getProductId, (row, p) -> {
    ...
    public PageInfo<ProductKnowledge> selectPage(ProductKnowledge productKnowledge, Integer pageNum, Integer pageSize) {
        ...
        // 走 selectAll 才能带上商品名/编号/封面的回填（分页列表与全量列表同一展示口径）
        List<ProductKnowledge> list = selectAll(productKnowledge);
