...
```
backend/
├── smartore-common          统一返回/错误码/全局异常(含校验异常)/用户上下文/Feign透传/AES/单号
...
| 集成 | `smartore-trade/app`、`smartore-goods/app` | Testcontainers 真 MySQL（+ 真 Redis）：Saga 六个分支、Mapper 绑定守卫、缓存命中与失效 |
| 评测 | `smartore-voice`（`-P eval`） | 需外部服务与真实 Key，默认 `@Tag("eval")` 排除 |
...
- **容器（app 栈）**：`deploy/compose.apps.yaml`（六个服务 + web，复用 mall 中间件网络），
  入口 `http://localhost:${SMARTORE_WEB_PORT:-8081}`
...
> 处置：`docker restart smartore-web` 让它重新解析。
